...
(function () {
  ...
  const isSafariMobile =
    (isSafari && (userAgent.indexOf("Mobile/") !== -1 || "standalone" in navigator)) || debug;
  ...
  function setup() {
    ...
    if (storedResponse) {
      try {
        ...
        console.warn("pwaGenerator error", err);
      }
      return;
    }
  ...
  function process(manifest, urlFactory) {
    ...
    const appleTouchIcons = (maskable.length > 0 ? maskable : icons)
      .map((icon) => {
    ...
    function renderBothSplash(applicationIcon, done) {
      ...
      // "bottlenecks" of pwaGenerator, so don't elongate any single frame more than needed.
  ...
  // actually run pwaGenerator here
